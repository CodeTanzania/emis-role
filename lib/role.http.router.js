 ...
 * @apiDefine Role Role
 *
 * @apiDescription A representation on how a party(or stakeholder) acts or,
 ...
 * @apiSuccess {String} [abbreviation] Human readable short form of a role.
...
router.get(PATH_LIST, function getRoles(request, response, next) {
  ...
  Role.get(options, function onGetRoles(error, results) {
    //forward error
    if (error) {
      next(error);
    }
...
router.post(PATH_LIST, function postRole(request, response, next) {
  ...
  Role.post(body, function onPostRole(error, created) {
    //forward error
    if (error) {
      next(error);
    }
...
router.get(PATH_SINGLE, function getRole(request, response, next) {
  ...
  Role.getById(options, function onGetRole(error, found) {
    //forward error
    if (error) {
      next(error);
    }
...
router.patch(PATH_SINGLE, function patchRole(request, response, next) {
  ...
  Role.patch(_id, patches, function onPatchRole(error, patched) {
    //forward error
    if (error) {
      next(error);
    }
...
router.put(PATH_SINGLE, function putRole(request, response, next) {
  ...
  Role.put(_id, updates, function onPutRole(error, updated) {
    //forward error
    if (error) {
      next(error);
    }
...
router.delete(PATH_SINGLE, function deleteRole(request, response, next) {
  ...
  Role.patch(_id, patches, function onDeleteRole(error, patched) {
    //forward error
    if (error) {
      next(error);
    }
